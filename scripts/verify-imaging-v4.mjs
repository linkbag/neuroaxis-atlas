...
for (const plate of anchored) {
  ...
  for (const other of sameAxis) {
    ...
    if (gap === 0) {
      fail('selection', `${plate.id} and ${other.id} are anchored to the same plane ${plate.planeValue}`)
    }
    if (gap <= tolerance) {
...
for (const plate of anchored) {
  const axis = AXIS_BY_SECTION[plate.axis]
  const reachable =
    plate.planeValue >= bounds[axis].min && plate.planeValue <= bounds[axis].max ? 'ok' : 'OUT OF RANGE'
  console.log(
