...
**AMENDMENT E — v17, the GRANULAR VASCULATURE (2026-09-20, measured against the working tree at `84c666b` + the v17 slice).** v8 gave the vasculature **14 artery records over 30 baked GLB parts**; v17 gives it **53 vessel records over 40 authored courses**, drawn as **procedural tubes** — the lenticulostriate arteries stop being schematic placement ellipsoids, and the intermediate/distal branches the v8 bake had merged into their parent's mesh become records of their own with their own courses. The run's contract is `PLAN.md` (architect) and its closure is `docs/SWARM_V17_PLAN.md`; §8 of `docs/VASC_INVENTORY.md` is the element-registration ledger.

| what | pinned revision (v14, `84c666b`) | current revision (v17, measured `npm run validate`) |
| --- | --- | --- |
| registry entries | 248 | **287** — vasculature **14 → 53**, every other region unchanged (telencephalon 87 · diencephalon 39 · midbrain 27 · pons 39 · medulla 37 · cerebellum 5) |
| registry by kind | nucleus 88 · tract 53 · ventricle 11 · surface 25 · **vessel 14** · context 45 · nerve 12 (7 kinds) | nucleus 88 · tract 53 · ventricle 11 · surface 25 · **vessel 53** · context 45 · nerve 12 (7 kinds) · Σ **287** |
| authored records | 225 in 19 files (0 awaiting a record) | **264 in 20 files** (0 awaiting a record) — the new file is `src/data/structures/vasculature-courses.json` |
| tracts / syndromes / plates / levels | 23 / 26 / 15 / 17 | **23 / 26 / 15 / 17** — unchanged |
| committed anatomy payload | 138 GLBs · 599,204 tris · 14,486,228 B (13.82 MiB) | **identical** — v17 adds **0 bytes** and **0 GLB files** |

### Where the granular course geometry lives, and what stands behind it (provenance)

| item | value |
| --- | --- |
| the collection | **39 records** in `src/data/structures/vasculature-courses.json` (37 new + 2 that replace built-in Tier-1 records by id), consumed through the merge in **`src/geometry/vasculature-courses.ts`** (`VesselCourseRecord`, `VESSEL_COURSES`, `mergeVesselCourses`) — a superset of the subset of `TractRecord` that `TractTube`/`tubeGeometryFor` read |
| **the built-in half** | `BUILT_IN_VESSEL_COURSES` in the same `.ts` file holds the **Tier-1 lenticulostriate courses** always in force (the umbrella `vasc-lenticulostriate-arteries` survives the merge; the lateral/medial group records are replaced by the authored ones), so the blob fix never depends on the JSON landing |
| **what each course is** | an **AUTHORED path** through documented landmarks: origin on a **literal committed parent-mesh vertex** (the M1 superior wall [15.786, 11.967, 24.920] is vertex 269 of `vasc-middle-cerebral-artery-m1-l`, exact distance 0.000 au), intermediate waypoints **projected onto a committed envelope**, and a terminal that **grazes** the structure it supplies (lateral 0.021–0.179 au from the putamen mesh, Heubner 0.249–0.296 au from the caudate). **Not segmented angiography, not tractography** — every record's `anchorNote` says so, and `surface: null` is only allowed with a stated reason (the perforators run *inside* the brain) |
| **the projection** | nearest mesh **vertex**, then the exact nearest point on its **incident triangles**, then an outward offset of `tubeRadius + 0.15 au`. **67 of the 172 waypoints** are projected; the gate re-derives every stored residual **from the GLB bytes** and cross-checks its accelerated search against an exhaustive pass over every triangle (24 probes, all exact) |
| **the radius** | `r_au = calibreMm / 2.4` at 1 au = 1.2 mm, from four **stated** calibres: 0.8 mm → 0.333 au (9 records, perforators), 1.0 mm → 0.417 (4), 1.2 mm → 0.500 (19, cortical branches), 2.0 mm → 0.833 (7, proximal segments). 39/39 satisfy `\|r × 2.4 − calibreMm\| < 0.03` (max error 0.0008 au). The drawn radius is a rendering of a stated calibre, not a measurement |
| **the length** | **1,775.64 au = 2,130.8 mm** of authored course (39 authored records 1,767.90 au = 2,121.5 mm; built-in umbrella 7.74 au = 9.3 mm), 172 waypoints, **all inside `CLIP_BOUNDS`**. Mirror: 52 mirrored projections re-checked, worst Δ **1.480 au = 1.78 mm** (tolerance 1.6 au — the two committed hemispheres are independently decimated) |
| **the surfaces hugged** | cortical branches → `ctx-hemisphere-l/r` (21 courses); SCA → `ctx-midbrain-surface` (3); AICA/pontine perforators → `ctx-pons-surface` (2); PICA → `ctx-cerebellum-l` (2); anterior spinal → `ctx-medulla-surface` (1); perforators → **none, stated** (lenticulostriate, thalamogeniculate, posteromedial central) |
| **what the 3D body is** | the project's existing `TractTube` sweep — 803 verts / 1,440 tris per tube, cached per id. 40 courses → **77 drawn tubes = 40 authored + 37 mirrored** (37 paired courses × 2 + 3 midline × 1) |
| **what the 2D body is** | procedurally generated parts: `SECTION_VESSEL_PARTS` + `registryVesselParts()` feed the **same** `tubeGeometryFor` output to the shipped contour worker. **77 worker parts** (the same two terms), **2,581 contour loops, 0 non-finite**. `partsForCanvas()` = **138 committed + 12 nerve + 40 vessel = 190 metas** |
| **what replaced what** | the **schematic placement ellipsoid is retired for every vessel with a course**: the shipped one-body-per-record table prints `tube only` for all 40 course records, `baked body` for the 13 base arteries with committed meshes, and **blobs 0** over all 53 vessel records (the 9 lenticulostriate ids are all suppressed). `hasVesselCourse(id)` is the suppression key |
| **new payload** | **0 bytes.** 138 manifest parts · 599,204 tris · `Σ stat(parts[].file)` = **14,486,228 B = 13.8151 MiB** · `src/assets/anatomy/` = **14,566,178 B = 13.8914 MiB** · **0** new GLBs · no bounding box moved. (Baking the branch elements was measured and rejected: **0.3–1.4 MiB** against **0.18 MiB** of headroom, decimated or not — `PLAN.md` §6) |
| **the gates** | `npm run verify:vasc-courses` — **2,171 assertions · 0 failures** (registration, bounds, the projection arithmetic re-derived from the mesh bytes, mirror consistency, links, the blob fix, the radius table, payload) · `npm run verify:vessel-render` — **75/75** (the merged table, the 3D tube list over 7 layer states, the one-body-per-record table, the 2D registry and worker contours, the shared builder, payload). Closures: `docs/SWARM_V17_PLAN.md` §2 (the 40-row table), §3 (the blob evidence), §5 (what did not ship) |

**Corrections of two v8 statements in this file and in the README.** v8 recorded that `vasc-lenticulostriate-arteries` "owns no mesh: it renders a **sized schematic placement marker**" and that "BP3D's MCA elements include the anterolateral central branches, but carving them out … would assert a segmentation the source does not make". Both are superseded: the record still owns no mesh, but it now **draws an authored perforator course** (`src/geometry/vasculature-courses.ts`), the MCA's anterolateral central elements (`FJ1662`/`FJ1663` + `M`) are named as the **basis** of the lateral chains without being carved into a baked body, and the two red ellipsoids no longer render.

**Open, carried forward (stated, not dropped).** (1) **`verify:cranial-nerve-render` is red 46/47** on this tree: its `partsForCanvas()` identity still reads `SECTION_PARTS + SECTION_NERVE_PARTS` and now finds **190**; the one-line re-point belongs to no v17 write scope. (2) **The 2D live section and the PiP do not paint the vessel contours yet**: `SectionCanvas.tsx`'s init-registry effect appends only `registryNerveParts()`; the vessel parts are computed but never handed to the worker. (3) **F1** `vasc-pica-tonsillomedullary-segment.territory` claims `vent-choroid-plexus-lateral` while its own `function` text says the **fourth** ventricle (its sibling claims `vent-fourth-ventricle` correctly). (4) **F2** `vasc-anterior-spinal-artery.territory` claims `ctx-pontine-fibers`, a pons row on a medullary artery. (5) **F3** the umbrella record's `contextNote` in `vasculature.json` still describes the retired ellipsoid and never states the authored-course basis; the **withdrawn** built-in medial course in `vasculature-courses.ts` still calls an ACA junction point the "carotid terminus". (6) **F4** the achieved surface clearance falls short of the intended `r + 0.15 au` in 61 of 67 placed waypoints (42 of them put the tube axis inside its own radius), because the offset uses an estimated rather than the nearest-triangle normal. Details, numbers and owners-unnamed fixes: `docs/SWARM_V17_PLAN.md` §5 and §11.
